...
15. **New command → README** — adding `.claude/commands/*.md` requires updating the Commands table in `README.md` in the same change. A command that exists but does not appear in the README is invisible to new users.
16. **No hardcoded absolute paths** — agent specs and command files must never contain absolute paths (`/Users/…`, `/home/…`). Use paths relative to the project root. The one exception is the user's own machine; even then, use `$PWD` or a note to run from the project root.
17. **New Bash call → settings.json** — when an agent or command adds a new shell invocation (`node`, `python3`, `kill`, `wc`, `curl`, etc.), add `"Bash(command:*)"` to `.claude/settings.json` allow-list in the same change. Missing entries cause a permission prompt on every pipeline run, breaking the flow.
18. **No CV_DATA keys hardcoded in fragments** — never hardcode values that are derivable from `CV_DATA` (category names, skill keys, KPI values, counts). Always derive dynamically: `CATEGORY_ORDER[0]` not `"languages"`, `computed.projects` not `8`. If a value comes from the data, the fragment must read it from the data.
19. **`_state.json` always committed as all-pending** — `_state.json` must be committed with every step set to `"pending"` and `current_step: 0`. Never commit a mid-pipeline state (`in_progress`, `validated`, etc.). Anyone cloning the repo must get a clean starting point.
20. **Verify assemble.mjs after touching it** — any edit to `scripts/assemble.mjs` must be followed by `node scripts/assemble.mjs --partial` before committing. If it exits with an error, the commit is not ready.
21. **Single source of truth for formulas** — when a formula (KPI computation, skill level mapping, date formatting) appears in more than one file, the files must be identical. When changing one, grep for all occurrences and update them all in the same commit.
